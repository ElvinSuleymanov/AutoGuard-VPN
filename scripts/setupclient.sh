...
# Token generating
    generate_password() { openssl rand -base64 12; }   #Pi-hole UI password
...
# Client scripts generation
    ...
    # Replaces all placeholders inside a file with actual runtime values.
    # Usage: inject_placeholders <file> <shell>
    inject_placeholders() {
        local file="$1"
        local shell="${2:-bash}"   # "bash" or "ps1"
    ...
    #Powershell scripting
        SCRIPT_POWERSHELL="./scripts/setupclient.ps1"
        inject_placeholders "$SCRIPT_POWERSHELL" "ps1"

    #Bash scripting
        SCRIPT_BASH="./scripts/setupclient.sh"
        inject_placeholders "$SCRIPT_BASH" "bash"
